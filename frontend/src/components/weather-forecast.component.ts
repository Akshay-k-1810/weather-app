...
export default class WeatherForecast extends Vue {
  ...
  formatTime(isoTime: Date) {
    ...
    return date.toLocaleDateString("en-US", options); // Use toLocaleTimeString for a human-readable time format
  }
  ...
  async fetchWeatherData(latitude: number, longitude: number) {
    try {
      ...
      this.forecastData = await this.weatherService.getWeatherForecast(
        latitude,
        longitude
      );
    } catch (error) {
